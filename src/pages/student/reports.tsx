...
export default function StudentReportsPage() {
  ...
  const renderDoc = (report: ElectronicReport) => {
    ...
    if (!exam) return null
    const rows = buildRows(student.id, report.examId, grades, subjects)
    const merit = meritRecords.filter((m) => m.studentId === student.id)
    ...
    return (
      <ElectronicReportDocument
        ...
        leadershipRole={student.leadershipRole}
      />
    )
  }
  ...
  return (
    <div>
      <PageHeader
        title="My Reports"
        description="Your published report cards. Select two terms to compare side by side."
      />

      {published.length === 0 ? (
        <EmptyState title="No published reports" description="Published reports will appear here when the school releases them." />
      ) : (
        <>
          <div className="mb-3">
            <Button size="sm" disabled={compareIds.length !== 2} onClick={() => setCompareOpen(true)}>
              <GitCompare className="mr-1.5 h-3.5 w-3.5" /> Compare ({compareIds.length}/2)
            </Button>
          </div>
