...
export function ElectronicReportDocument({
  ...
  return (
    <div
      className={cn(
        "print-area rounded-lg border-2 border-navy-800/20 bg-card p-6 shadow-soft sm:p-8",
        className,
      )}
    >
        ...
        <div className="relative flex items-center justify-between gap-4">
          ...
          <div className="text-right">
            ...
            <div className="mt-1.5 flex justify-end">
              <LeadershipBadge
                role={leadershipRole ?? student.leadershipRole}
              />
            </div>
          </div>
        </div>
      ...
      <table className="mt-5 w-full border-collapse text-sm">
        <thead>
          <tr className="border-y bg-navy-50 text-left">
            ...
            <th className="px-3 py-2 text-center font-semibold text-navy-700">
              Grade
            </th>
            <th className="px-3 py-2 font-semibold text-navy-700">
              Teacher Comment
            </th>
          </tr>
        </thead>
      ...
      {predictions.length > 0 && (
        <div className="mt-4 rounded-md border border-dashed border-navy-300 bg-muted/40 p-3">
          <p className="text-xs font-semibold uppercase tracking-wide text-navy-700 mb-2">
            Projected Next Term
          </p>
            ...
            {predictions.map((p) => (
              <li key={p.subjectId}>
                <span className="font-medium">
                  {subjectNames[p.subjectId] ?? p.subjectId}
                </span>
                {": "}
      ...
      <p className="mt-4 text-center text-[10px] text-muted-foreground">
        {exam.name} · Issued{" "}
        {report.publishedAt ? formatDate(report.publishedAt.slice(0, 10)) : "—"}{" "}
        · Westwood College, Harare
      </p>
    </div>
  );
}
